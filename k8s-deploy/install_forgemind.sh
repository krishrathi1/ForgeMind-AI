...
echo "Deploying production ForgeMind (using external databases)..."
...
helm upgrade --install forgemind $SCRIPT_DIR/forgemind \
  --namespace $NAMESPACE \
...
echo ""
echo "Waiting for forgemind pod to be ready..."
kubectl wait --for=condition=ready pod -l app.kubernetes.io/instance=forgemind --timeout=300s -n rag
echo "forgemind pod is ready"
echo ""
echo "Running Port-Forward:"
echo "    kubectl --namespace rag port-forward svc/forgemind 9621:9621"
echo "==========================================="
echo ""
