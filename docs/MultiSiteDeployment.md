...
This document explains how to run multiple isolated ForgeMind instances behind one host using a reverse proxy (nginx, Traefik, Kubernetes Ingress, …), with **one shared WebUI build** reused by every instance.
...
- Set `FORGEMIND_API_PREFIX` per-instance, on the **backend only**. The WebUI is always mounted at `/webui` (not configurable).
...
docker run -e FORGEMIND_API_PREFIX=/site01 -p 9621:9621 forgemind:latest
docker run -e FORGEMIND_API_PREFIX=/site02 -p 9622:9621 forgemind:latest
```
...
Earlier versions of ForgeMind baked the site prefix into the JavaScript bundle at build time (via `VITE_API_PREFIX` / `VITE_WEBUI_PREFIX`). Every site that used a different prefix needed its own WebUI build, and reusing a single Docker image across sites required a rebuild step at deploy time. Since the runtime-config-injection refactor:
...
- **API base URL** and **in-app links** read their prefix from `window.__FORGEMIND_CONFIG__`, which the FastAPI server injects into `index.html` on each response based on its own `FORGEMIND_API_PREFIX`.

The result: a single `forgemind/api/webui/` directory (or Docker image) is reusable across any number of sites with no per-site build artifact.

---

## How runtime prefix injection works

Each request for `index.html` goes through `SmartStaticFiles` in `forgemind/api/forgemind_server.py`, which:

1. Reads the static `index.html` produced by `bun run build`.
2. Looks for the placeholder comment `<!-- __FORGEMIND_RUNTIME_CONFIG__ -->`.
3. Replaces it with
   `<script>window.__FORGEMIND_CONFIG__ = {"apiPrefix":"…","webuiPrefix":"…"}</script>`,
   computed from the configured `FORGEMIND_API_PREFIX` (the in-app `/webui` mount is hardcoded server-side).
...
| --- | --- | --- |
| `FORGEMIND_API_PREFIX` | `""` | Reverse-proxy mount prefix. The backend accepts both strip and verbatim forwarding — pick whichever fits your proxy stack. Passed to FastAPI as `root_path`. |

The WebUI is always mounted at `/webui` server-side. `window.__FORGEMIND_CONFIG__.webuiPrefix` is computed as `FORGEMIND_API_PREFIX + "/webui/"` and injected for the SPA — you do **not** set it yourself.
...
After setting `FORGEMIND_API_PREFIX=/site01`, the backend resolves all routes correctly under either forwarding style:
...
FORGEMIND_API_KEY=site01-secret
...
services:
  site01:
    image: ghcr.io/krishrathi1/forgemind-ai:latest
    env_file: site01.env
    volumes:
...
WORKDIR /src/forgemind_webui
COPY forgemind_webui/package.json forgemind_webui/bun.lock ./
RUN bun install --frozen-lockfile
COPY forgemind_webui/ ./
COPY forgemind/api/webui/.gitkeep /src/forgemind/api/webui/.gitkeep
RUN bun run build

FROM python:3.11-slim
COPY --from=webui-build /src/forgemind/api/webui /app/forgemind/api/webui
# … rest of the image …
```
...
docker run --rm -p 9621:9621 forgemind:latest

# Same image, different prefixes — runtime decides.
docker run --rm -e FORGEMIND_API_PREFIX=/site01 -p 9621:9621 forgemind:latest
docker run --rm -e FORGEMIND_API_PREFIX=/site02 -p 9622:9621 forgemind:latest
```
  ...
  - host: host.example.com
    http:
      ...
      - path: /site01(/|$)(.*)
        ...
        backend:
          service:
            name: forgemind-site01
            port: { number: 9621 }
      - path: /site02(/|$)(.*)
        pathType: ImplementationSpecific
        backend:
          service:
            name: forgemind-site02
            port: { number: 9621 }
```

Backends still set `FORGEMIND_API_PREFIX=/site01` / `=/site02`.
...
>
> This is the deliberate consequence of `base: './'` in [`vite.config.ts`](../forgemind_webui/vite.config.ts) — the same setting that makes one production build reusable across any number of reverse-proxy mount points. Tying the dev URL to a prefix would force the build to bake the prefix back in.

The dev server mirrors production injection: it serves `index.html` via the same `transformIndexHtml` mechanism the FastAPI server uses at request time, so the SPA reads `window.__FORGEMIND_CONFIG__` in dev exactly the way it does in prod. Only **two** environment variables matter:

| Variable | Purpose | Where it lives |
| --- | --- | --- |
| `VITE_BACKEND_URL` | Where the dev server forwards proxied API calls. | `forgemind_webui/.env*` |
| `VITE_DEV_API_PREFIX` | Prefix to **simulate** (matches the backend FORGEMIND_API_PREFIX`). Empty → no prefix. | `forgemind_webui/.env*` |

`VITE_DEV_API_PREFIX` injects `apiPrefix` into `window.__FORGEMIND_CONFIG__` in the browser, mirroring the backend behavior. It also serves as a prefix for `VITE_API_ENDPOINTS`, ensuring correct access to backend APIs. The matching `webuiPrefix` is derived as `${VITE_DEV_API_PREFIX}/webui/` automatically — you don't need a separate variable for it.
...
```bash
# forgemind_webui/.env.development (already in repo as sample)
...
Run:
```bash
forgemind-server                  # in one terminal, no FORGEMIND_API_PREFIX
cd forgemind_webui && bun run dev # in another; open http://localhost:5173/
```

### Scenario 2 — simulate a site prefix

You want the SPA to run under `/site01` (or whatever production prefix). Set `VITE_DEV_API_PREFIX=/site01`. Vite injects the matching `window.__FORGEMIND_CONFIG__` and registers prefixed proxy keys; SPA requests like `fetch("/site01/documents/foo")` are forwarded verbatim to whatever `VITE_BACKEND_URL` points at. The upstream — local backend or production nginx — is responsible for understanding the prefix.
...
**A. Local backend with `FORGEMIND_API_PREFIX=/site01`** (no nginx anywhere) — the simplest setup, two processes on your laptop. Vite's proxy itself plays the role of the reverse proxy.
...
```bash
# Terminal 1
FORGEMIND_API_PREFIX=/site01 forgemind-server
# Terminal 2
cd forgemind_webui && bun run dev
```
...
- **Auth required**: if the upstream requires `FORGEMIND_API_KEY`, log in via the dev SPA exactly as you would in prod — the auth token flows through the proxy unchanged.
...
| 2A. Simulate a prefix locally (no nginx) | `http://localhost:9621` | `/site01` | local backend with `FORGEMIND_API_PREFIX=/site01` | `http://localhost:5173/` |
...
### `apiPrefix` is empty in `window.__FORGEMIND_CONFIG__` after deploy

View the page source. If you see the literal placeholder `<!-- __FORGEMIND_RUNTIME_CONFIG__ -->` instead of an injected `<script>` tag, the request did not go through `SmartStaticFiles` — double-check that `forgemind/api/webui/index.html` exists in the running container and that the WebUI mount succeeded (the server logs `WebUI assets mounted at <path>` at startup).

### `bun run dev` proxy returns 404 with `VITE_DEV_API_PREFIX` set

Confirm the backend is also running with the matching `FORGEMIND_API_PREFIX`. The dev proxy forwards prefixed paths verbatim; if the backend has no prefix configured, it does not register routes under that path.

### I want to disable the WebUI entirely

Don't build the frontend — `forgemind/api/webui/index.html` will not exist and the server will skip the WebUI mount, redirecting `/` and the WebUI path to `/docs` instead. The runtime-config injection is purely opt-in via the existence of the build artifact.
