# ForgeMind Server and WebUI

The ForgeMind Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. ForgeMind Server also provides an Ollama-compatible interface, aiming to emulate ForgeMind as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access ForgeMind easily.

![image-20250323122538997](./ForgeMind-API-Server.assets/image-20250323122538997.png)

![image-20250323122754387](./ForgeMind-API-Server.assets/image-20250323122754387.png)

![image-20250323123011220](./ForgeMind-API-Server.assets/image-20250323123011220.png)

## Upgrading from v1.4.16 to v1.5.0rc2

The v1.5.0rc2 release adds the new file-processing pipeline, parser routing, multimodal analysis, role-specific LLM/VLM configuration, JSON entity extraction, and several provider/storage changes. Review the [v1.5.0rc2 release notes](https://github.com/krishrathi1/ForgeMind-AI/releases/tag/v1.5.0rc2) before upgrading a production instance.
...
- Changing the embedding model, embedding dimension, asymmetric embedding behavior, or query/document prefixes changes vector semantics. Clear the affected ForgeMind workspace/vector data and re-index source files.
- Changing parser routing (`FORGEMIND_PARSER`) or filename hints affects newly uploaded files. To switch an existing document to another parser engine, delete that document and upload it again.
...
### Before Starting ForgeMind Server

ForgeMind necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the ForgeMind server, it is essential to configure the settings for both the LLM and the Embedding Model.

ForgeMind supports these LLM backends:
...
ForgeMind supports these embedding backends:
...
It is recommended to use environment variables to configure the ForgeMind Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the ForgeMind Server will load the environment variables from `.env` into the system environment variables each time it starts. **ForgeMind Server will prioritize the settings in the system environment variables to .env file**.
...
> **Important Note**: The embedding model and asymmetric embedding configuration must be determined before document indexing, and the same settings must be used during the query phase. For certain storage solutions (e.g., PostgreSQL), the vector dimension must be defined upon initial table creation. When changing the embedding model, embedding dimension, `EMBEDDING_ASYMMETRIC`, query/document prefixes, or provider task behavior, clear the existing ForgeMind workspace/vector data and re-index the source files.

#### Asymmetric Embedding Configuration

ForgeMind uses symmetric embeddings by default. Query/document asymmetric embeddings are enabled only when `EMBEDDING_ASYMMETRIC=true` is explicitly set.
...
### Starting ForgeMind Server

The ForgeMind Server supports two operational modes:
...
```
forgemind-gunicorn --workers 4
```

When starting ForgeMind, the current working directory must contain the `.env` configuration file. **It is intentionally designed that the `.env` file must be placed in the startup directory**. The purpose of this is to allow users to launch multiple ForgeMind instances simultaneously and configure different `.env` files for different instances. **After modifying the `.env` file, you need to reopen the terminal for the new settings to take effect.** This is because each time ForgeMind Server starts, it loads the environment variables from the `.env` file into the system environment variables, and system environment variables have higher precedence.
...
- `--workspace`: Workspace name, used to logically isolate data between multiple ForgeMind instances (default: empty)
- `--api-prefix`: Reverse-proxy path prefix exposed to browsers, also configurable with `FORGEMIND_API_PREFIX`
- `--rerank-binding`: Rerank provider (`null`, `cohere`, `jina`, or `aliyun`)

### Path Prefix and Multi-Site WebUI

Set `FORGEMIND_API_PREFIX` or `--api-prefix` when one host serves multiple ForgeMind instances behind a reverse proxy that strips a site prefix before forwarding to the backend:

```bash
FORGEMIND_API_PREFIX=/site01
forgemind-server --port 9621
```
...
### Launching ForgeMind Server with Docker

Using Docker Compose is the most convenient way to deploy and run the ForgeMind Server.

- Create a project directory.
- Copy the `docker-compose.yml` file from the ForgeMind repository into your project directory.
...
- Start the ForgeMind Server with the following command:
...
You can get the official docker compose file from here: [docker-compose.yml](https://raw.githubusercontent.com/krishrathi1/ForgeMind-AI/refs/heads/main/docker-compose.yml). For historical versions of ForgeMind docker images, visit this link: [ForgeMind Docker Images](https://github.com/krishrathi1/ForgeMind-AI/pkgs/container/forgemind). For more details about docker deployment, please refer to [DockerDeployment.md](./DockerDeployment.md).

### Progressive Setup Recipes

If you are new to ForgeMind, start with the smallest working configuration and add capabilities only after the previous step is healthy:
...
###########################
PORT=9621
WEBUI_TITLE='My First ForgeMind KB'
...
If ForgeMind itself runs inside Docker and the reranker runs on the host, use a host-reachable address such as `host.docker.internal` instead of `localhost`. If the setup wizard creates the vLLM service, it injects the internal Compose service URL into `docker-compose.final.yml` for you.

#### 3. Add Multimodal Parsing With MinerU Official API

Use this after the basic document flow works. The MinerU official API avoids running a local parser service, but `MINERU_API_TOKEN` must be configured before the ForgeMind server starts. The VLM role must use a provider/model that supports image input.
...
- `FORGEMIND_GRAPH_STORAGE=MemgraphStorage`
...
- Changing `FORGEMIND_PARSER` affects only newly uploaded files. Delete and upload an existing document again if you want it processed by a different parser route.

### Nginx Reverse Proxy Configuration

When using Nginx as a reverse proxy in front of ForgeMind Server, you need to configure `client_max_body_size` for the `/documents/upload` endpoint to handle large file uploads. Without this configuration, Nginx will reject files larger than 1MB (the default limit) with a `413 Request Entity Too Large` error before the request reaches ForgeMind.
...
3. **Streaming Endpoints**: Disable gzip compression (`gzip off`) for streaming endpoints to ensure real-time response delivery. ForgeMind automatically sets `X-Accel-Buffering: no` header to disable response buffering.
...
5. **Size Validation Layers**:
   ...
   - ForgeMind performs streaming validation during upload
   - Setting appropriate limits at both layers ensures better error messages and security

### Offline Deployment

Official ForgeMind Docker images are fully compatible with offline or air-gapped environments. If you want to build up your own offline environment, please refer to [Offline Deployment Guide](./OfflineDeployment.md).

### Starting Multiple ForgeMind Instances

There are two ways to start multiple ForgeMind instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `forgemind-server` in the working directory.

The second way is for all instances to share the same set of `.env` configuration files, and then use command-line arguments to specify different server listening ports and workspaces for each instance. You can start multiple ForgeMind instances in the same working directory with different command-line arguments. For example:

```
# Start instance 1
forgemind-server --port 9621 --workspace space1

# Start instance 2
forgemind-server --port 9622 --workspace space2
```
...
The ForgeMind Server can operate in the `Gunicorn + Uvicorn` preload mode. Gunicorn's multiple worker (multiprocess) capability prevents document indexing tasks from blocking RAG queries. CPU-heavy document extraction tools should be deployed as external services so they do not block the API process.

Though ForgeMind Server uses one worker to process the document indexing pipeline, with the async task support of Uvicorn, multiple files can be processed in parallel. The bottleneck of document indexing speed mainly lies with the LLM. If your LLM supports high concurrency, you can accelerate document indexing by increasing the concurrency level of the LLM. Below are several environment variables related to concurrent processing, along with their default values:
...
forgemind-gunicorn --workers 2
```

### Install ForgeMind as a Linux Service

Create your service file `forgemind.service` from the sample file: `forgemind.service.example`. Modify the start options the service file:

```text
# Set environment to your Python virtual environment
Environment="PATH=/home/netman/forgemind-xyj/venv/bin"
WorkingDirectory=/home/netman/forgemind-xyj
# ExecStart=/home/netman/forgemind-xyj/venv/bin/forgemind-server
ExecStart=/home/netman/forgemind-xyj/venv/bin/forgemind-gunicorn
```

> The ExecStart command must be either `forgemind-gunicorn` or `forgemind-server`; no wrapper scripts are allowed. This is because service termination requires the main process to be one of these two executables.

Install ForgeMind service. If your system is Ubuntu, the following commands will work:

```shell
sudo cp forgemind.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start forgemind.service
sudo systemctl status forgemind.service
sudo systemctl enable forgemind.service
```

## Ollama Emulation

We provide Ollama-compatible interfaces for ForgeMind, aiming to emulate ForgeMind as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access ForgeMind easily.

### Connect Open WebUI to ForgeMind

After starting the forgemind-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `forgemind:latest` will appear in Open WebUI's model management interface. Users can then send queries to ForgeMind through the chat interface. You should install ForgeMind as a service for this use case.
...
![image-20250323194750379](./ForgeMind-API-Server.assets/image-20250323194750379.png)

### Choose Query mode in chat

The default query mode is `hybrid` if you send a message (query) from the Ollama interface of ForgeMind. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which ForgeMind query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's ForgeMind?` will trigger a mix mode query for ForgeMind. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a ForgeMind query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a ForgeMind query mode; it will tell ForgeMind to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using ForgeMind for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the ForgeMind Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.

* API Key:

```
FORGEMIND_API_KEY=your-secure-api-key-here
...
The API key is passed using the request header `X-API-Key`. Below is an example of accessing the ForgeMind Server via API:
...
ForgeMind API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
```bash
forgemind-hash-password --username admin
```
...
RESOURCE_GROUP_NAME=ForgeMind
LOCATION=swedencentral
RESOURCE_NAME=ForgeMind-OpenAI
...
## ForgeMind Server Configuration in Detail
...
ForgeMind supports binding to various LLM backends:
...
ForgeMind supports binding to various Embedding backends:
...
```
forgemind-server --llm-binding openai --help
forgemind-server --llm-binding ollama --help
forgemind-server --llm-binding gemini --help
forgemind-server --embedding-binding ollama --help
forgemind-server --embedding-binding gemini --help
```
...
PROMPT_DIR=/opt/forgemind/prompts
...
```
FORGEMIND_KV_STORAGE=PGKVStorage
FORGEMIND_VECTOR_STORAGE=PGVectorStorage
FORGEMIND_GRAPH_STORAGE=PGGraphStorage
FORGEMIND_DOC_STATUS_STORAGE=PGDocStatusStorage
```

You cannot change storage implementation selection after adding documents to ForgeMind. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample `.env.example` file.

### LLM Cache Migration Between Storage Types

When switching the storage implementation in ForgeMind, the LLM cache can be migrated from the existing storage to the new one. Subsequently, when re-uploading files to the new storage, the pre-existing LLM cache will significantly accelerate file processing. For detailed instructions on using the LLM cache migration tool, please refer to [README_MIGRATE_LLM_CACHE.md](../forgemind/tools/README_MIGRATE_LLM_CACHE.md)

### ForgeMind API Server Command Line Options
...
| `--api-prefix` | `""` | Reverse-proxy path prefix, also configurable with `FORGEMIND_API_PREFIX` |
...
Reranking query-recalled chunks can significantly enhance retrieval quality by re-ordering documents based on an optimized relevance scoring model. ForgeMind currently supports the following rerank providers:
...
```json
{
  "response": "ForgeMind is a graph-based RAG system...",
  "references": [
    {
      "reference_id": "1",
      "file_path": "/documents/intro.md",
      "content": [
        "ForgeMind is a retrieval-augmented generation system that combines knowledge graphs with vector similarity search...",
...
### For JWT Auth
# AUTH_ACCOUNTS='admin:{bcrypt}$2b$12$replace-with-generated-hash,user1:pass456'
# TOKEN_SECRET=your-key-for-ForgeMind-API-Server-xxx
# TOKEN_EXPIRE_HOURS=48

# FORGEMIND_API_KEY=your-secure-api-key-here-123
# WHITELIST_PATHS=/api/*
# WHITELIST_PATHS=/health,/api/*
```
...
`FORGEMIND_PARSER` defines default extraction rules by file extension. Rules are matched left to right and can be separated by commas or semicolons:
...
| `native` | Built-in structured parser, currently focused on `.docx` and ForgeMind Document sidecars. |
...
The `/documents/upload` and `/documents/scan` paths honor filename hints and `FORGEMIND_PARSER`. The `/documents/text` and `/documents/texts` endpoints insert already-provided text and currently use fixed chunking on the server path.
...
| `P` | Paragraph semantic chunking for structured ForgeMind Document content; falls back to `R` when structured content is unavailable |
...
All supported backends (`lollms`, `ollama`, `openai` / OpenAI-compatible, `azure_openai`, `bedrock`, and `gemini`) expose the same ForgeMind REST API surface. When the API Server is running, visit:
...
ForgeMind implements asynchronous document indexing to enable frontend monitoring and querying of document processing progress. Upon uploading files or inserting text through designated endpoints, a unique Track ID is returned to facilitate real-time progress monitoring.
