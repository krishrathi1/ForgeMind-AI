...
ForgeMind 支持为不同处理阶段配置不同的 LLM 或 VLM。这个机制适合把低成本模型用于抽取，把更强模型用于最终回答，或为多模态分析单独指定视觉语言模型。
...
如果某个角色没有专门配置，ForgeMind 会使用基础 `LLM_*` 配置。
...
- 如果没有设置 `{ROLE}_LLM_BINDING_HOST`，ForgeMind 会尝试使用该 provider 的默认 host。
...
这个模式不是跨 provider，因为三个角色的 binding 都是 `openai`。ForgeMind 会分别把每个角色的 `*_LLM_BINDING_HOST` 和 `*_LLM_BINDING_API_KEY` 传给 OpenAI-compatible client。
...
- Gemini Vertex AI 模式由进程级 Google 环境变量控制，不能在同一个 ForgeMind 进程里让某些角色使用 Vertex AI、另一些角色使用 AI Studio API key。
- `LLM_BINDING_HOST` 在 Docker/Compose 中通常需要使用容器可访问地址，例如 `host.docker.internal`，角色级 host 也遵循相同原则。
- 修改 `.env` 后请重启 ForgeMind Server。部分 IDE 终端会预加载 `.env`，建议打开新的终端会话确认环境变量生效。
