...
COPY forgemind_webui/ ./forgemind_webui/
...
COPY forgemind/ ./forgemind/

# Include pre-built frontend assets from the previous stage
COPY --from=frontend-builder /app/forgemind/api/webui ./forgemind/api/webui
...
RUN mkdir -p /app/data/tiktoken \
    && uv run forgemind-download-cache --cache-dir /app/data/tiktoken --spacy --spacy-dir /app/spacy_models || status=$?; \
...
COPY --from=builder /app/forgemind ./forgemind
COPY pyproject.toml .
COPY setup.py .
COPY uv.lock .
...
RUN apt-get update \
    ...
    && rm -rf /var/lib/apt/lists/* \
    && groupadd -g 1000 forgemind \
    && useradd -u 1000 -g forgemind -m -d /home/forgemind -s /usr/sbin/nologin forgemind \
    && chown -R forgemind:forgemind /app /home/forgemind
...
ENV HOME=/home/forgemind \
    XDG_CACHE_HOME=/home/forgemind/.cache \
    PIP_CACHE_DIR=/home/forgemind/.cache/pip \
    UV_CACHE_DIR=/home/forgemind/.cache/uv

# Entrypoint starts as root, fixes mount ownership, then drops to forgemind.
...
CMD ["python", "-m", "forgemind.api.forgemind_server"]
