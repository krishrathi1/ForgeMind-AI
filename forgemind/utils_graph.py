...
async def _merge_entities_impl(
    ...
    *,
    ...
    Failure semantics:
        ...
        rebuild tool (``forgemind-rebuild-vdb``) restores full consistency.
    """
    ...
    if relations_to_delete:
        try:
            ...
            raise VectorStorageConsistencyError(
                ...
                "source and the source entities were not deleted). Stop the ForgeMind server "
                "and run the offline rebuild tool (forgemind-rebuild-vdb) to restore "
                "consistency."
            ) from e
    ...
    for rel_data in relation_updates.values():
        ...
        try:
            ...
            raise VectorStorageConsistencyError(
                ...
                "and the source entities were not deleted). Stop the ForgeMind server and run "
                "the offline rebuild tool (forgemind-rebuild-vdb) to restore consistency."
            ) from e
        logger.debug(
    ...
    try:
        ...
        raise VectorStorageConsistencyError(
            ...
            "authoritative source and the source entities were not deleted). Stop the ForgeMind "
            "server and run the offline rebuild tool (forgemind-rebuild-vdb) to restore consistency."
        ) from e
    ...
    for entity_name in source_entities:
        ...
        try:
            ...
            raise VectorStorageConsistencyError(
                ...
                "data is lost. Stop the ForgeMind server and run the offline rebuild "
                "tool (forgemind-rebuild-vdb) to clear the stale record and restore "
                "consistency."
            ) from e
    ...
    try:
        ...
        raise VectorStorageConsistencyError(
            ...
            "No data is lost. Stop the ForgeMind server and run the offline rebuild tool "
            "(forgemind-rebuild-vdb) to restore consistency."
        ) from e
