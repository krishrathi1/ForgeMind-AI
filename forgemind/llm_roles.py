...
ForgeMind can route different stages of work (entity extraction, keyword
...
class RoleSpec:
    ...
    Adding a new role anywhere in ForgeMind is a single-line edit: append a
...
@dataclass
class RoleLLMConfig:
    """Per-role LLM override accepted at :class:`ForgeMind` init time.
...
class _RoleLLMMixin:
    """Mixin that owns the role LLM runtime on :class:`ForgeMind`.
