...
to :data:`forgemind.constants.DEFAULT_R_SEPARATORS`); only when a single
...
:func:`forgemind.chunk_schema.strip_internal_multimodal_markup_for_extraction`
...
``forgemind.operate._process_single_content``) and ensures the
...
from forgemind.chunk_schema import strip_internal_multimodal_markup_for_extraction
...
from forgemind.utils import Tokenizer
...
def load_chunk_separators() -> list[str]:
    ...
    :data:`forgemind.constants.DEFAULT_R_SEPARATORS` on missing / invalid
...
def trim_content_to_budget(
    ...
    *,
    ...
    :data:`forgemind.constants.DEFAULT_MAX_EXTRACT_INPUT_TOKENS`.  Only ``content``
