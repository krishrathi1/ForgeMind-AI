...
Why this lives at the package root rather than under ``forgemind/kg/``:
``forgemind.utils.write_json`` needs ``atomic_write`` to gain crash safety,
and several ``forgemind/kg/*`` modules need both. Hosting the helpers under
``forgemind/kg/`` would create a ``utils -> kg -> utils`` import cycle.
