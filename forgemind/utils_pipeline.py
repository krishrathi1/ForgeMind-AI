...
These helpers are shared by the ForgeMind pipeline mixin (forgemind/pipeline.py)
and by other ForgeMind methods that touch the document ingestion paths
...
from forgemind.base import DocProcessingStatus, DocStatus, DocStatusStorage
...
from forgemind.parser.routing import canonicalize_parser_hinted_basename
...
def resolve_doc_status_parse_engine(
    ...
    structured ``forgemind`` format and ``legacy`` for everything else
    ...
    return (
        PARSER_ENGINE_NATIVE
        if parse_format == FULL_DOCS_FORMAT_FORGEMIND
...
def compute_file_content_hash(path_str: str) -> str | None:
    ...
    Resolves the ForgeMind ``*.blocks.jsonl`` conventions used by
    ``_load_forgemind_document_content`` so the hash matches the actual
...
def make_forgemind_doc_content(merged_text: str) -> str:
    """Build the ``full_docs.content`` value for ``format=forgemind`` records.

    The result has shape ``"{{LRdoc}}<merged_text>"`` — the marker prefix
    distinguishes forgemind-format full_docs from raw-format ones, and the
    body is the complete merged text from the ``.blocks.jsonl`` content
    lines so F-chunking can run identically on raw and forgemind inputs
    (the prefix is stripped at chunking time via
    ``strip_forgemind_doc_prefix``).
    """
    return f"{FORGEMIND_DOC_CONTENT_PREFIX}{merged_text or ''}"


def strip_forgemind_doc_prefix(content: str | None, parse_format: str | None) -> str:
    """Return the bare body for a stored ``full_docs.content`` value.

    The ``{{LRdoc}}`` marker is stripped **only** when ``parse_format``
    indicates the record is in forgemind format.  Any other ``parse_format``
    ...
    if (
        ...
        and content.startswith(FORGEMIND_DOC_CONTENT_PREFIX)
    ):
        return content[len(FORGEMIND_DOC_CONTENT_PREFIX) :]
    return content or ""
...
# ---------------------------------------------------------------------------
# ForgeMind Document blocks loader
# ---------------------------------------------------------------------------


async def load_forgemind_document_content(sidecar_uri: str) -> tuple[str, str]:
    """Load ForgeMind Document blocks and return ``(merged_text, blocks_path)``.
    ...
    if resolved is None:
        raise FileNotFoundError(
            f"ForgeMind blocks file not found from sidecar uri: {sidecar_uri}"
        )
...
# :func:`forgemind.operate.extract_entities._process_single_content` so each
