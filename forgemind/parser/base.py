...
(:mod:`forgemind.parser.registry`) instead of a growing ``if engine == …``
chain.
...
if TYPE_CHECKING:
    from forgemind.sidecar.ir import IRDoc  # noqa: F401
...
class ParseContext:
    ...
    Wraps the ForgeMind instance plus the per-document handles so a parser
    ...
    def source_path(self, parser_engine: str) -> Path:
        ...
        from forgemind.pipeline import (
    ...
    async def archive_source(self, source_path: str) -> str | None:
        ...
        ``forgemind.pipeline.archive_docx_source_after_full_docs_sync`` keep
