...
Like :data:`forgemind.kg.STORAGES` / ``STORAGE_IMPLEMENTATIONS``, this module
...
if TYPE_CHECKING:
    from forgemind.parser.base import BaseParser
...
class ParserSpec:
    ...
    # mirroring how the built-in ``max_parallel_parse_*`` ForgeMind fields read
    # their env. ``None`` means this spec does not own its group's concurrency
    # (built-in groups are sized by the ForgeMind instance field instead).
...
_REGISTRY: dict[str, ParserSpec] = {
    PARSER_ENGINE_NATIVE: ParserSpec(
        ...
        # source suffix (see forgemind.parser.native_dispatch).
        impl="forgemind.parser.native_dispatch:NativeParser",
        suffixes=frozenset({"docx", "md", "textpack"}),
        queue_group=PARSER_ENGINE_NATIVE,
        # Built-in groups are sized by the ForgeMind ``max_parallel_parse_*``
    ...
    PARSER_ENGINE_LEGACY: ParserSpec(
        ...
        impl="forgemind.parser.legacy.parser:LegacyParser",
    ...
    PARSER_ENGINE_MINERU: ParserSpec(
        ...
        impl="forgemind.parser.external.mineru.parser:MinerUParser",
    ...
    PARSER_ENGINE_DOCLING: ParserSpec(
        ...
        impl="forgemind.parser.external.docling.parser:DoclingParser",
    ...
    PARSER_ENGINE_REUSE: ParserSpec(
        ...
        impl="forgemind.parser.noop:ReuseParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
    PARSER_ENGINE_PASSTHROUGH: ParserSpec(
        engine_name=PARSER_ENGINE_PASSTHROUGH,
        impl="forgemind.parser.noop:PassthroughParser",
        suffixes=frozenset(),
        user_selectable=False,
    ),
}
