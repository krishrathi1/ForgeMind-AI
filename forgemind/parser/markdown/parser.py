...
from forgemind.constants import NATIVE_RAW_DIR_SUFFIX, PARSER_ENGINE_NATIVE
from forgemind.parser.external._common import raw_dir_for_parsed_dir
from forgemind.parser.markdown.extract import ResolvedImage, extract_markdown
from forgemind.parser.markdown.raw_cache import (
    NativeImageRawCache,
    native_md_options_signature,
)
from forgemind.parser.native_base import NativeExtractRuntime, NativeParserBase
from forgemind.utils import logger

if TYPE_CHECKING:
    from forgemind.sidecar.ir import IRDoc
...
class _MarkdownImageResolver:
    """Concrete :class:`~forgemind.parser.markdown.extract.ImageResolver`.
    ...
    def _download(self, src: str) -> tuple[bytes, str]:
        ...
        req = urllib.request.Request(src, headers={"User-Agent": "forgemind-native-md"})
...
class NativeMarkdownParser(NativeParserBase):
    ...
    def extract(
        ...
        *,
        ...
        # rule like ``FORGEMIND_PARSER=native(smart_heading=true)`` doesn't hard
        ...
        raw_cache = NativeImageRawCache(
            ...
            force_reparse=_env_bool("FORGEMIND_FORCE_REPARSE_NATIVE", False),
        )
        raw_cache.load()
    ...
    def _open_textpack(self, source: Path, tmp_dir: Path) -> tuple[str, Path]:
        from forgemind.parser.external._zip import safe_extract_zip
