...
from forgemind.utils import logger, parse_optional_float
...
# there automatically (no filename hint / FORGEMIND_PARSER rule needed). ``.md``
...
class ParserRoutingConfigError(ValueError):
    """Raised when FORGEMIND_PARSER contains an invalid routing rule."""
...
class ProcessOptions:
    ...
    def chunking_explicit(self) -> bool:
        ...
        legacy externally-supplied :attr:`ForgeMind.chunking_func`.
        """
...
def default_chunker_config() -> dict[str, Any]:
    ...
    *strategy-specific* env vars (``CHUNK_F_SIZE``,
    ...
    :meth:`ForgeMind._apply_chunk_size_overlay` apply the legacy
    constructor field (``ForgeMind(chunk_overlap_token_size=…)``) and
...
def resolve_chunk_options(
    ...
    *,
    ...
    The F runtime args from ``ForgeMind.ainsert`` overlay the
...
def parser_rules_from_env() -> str:
    return os.getenv("FORGEMIND_PARSER", "").strip()
...
def _rule_engine_and_options(engine_hint: str) -> tuple[str, str]:
    """Split a ``FORGEMIND_PARSER`` rule's RHS (``engine[-options]``).
    ...
    boundary, since ``FORGEMIND_PARSER`` rules cannot be options-only.
    """
...
def validate_parser_routing_config(parser_rules: str | None = None) -> None:
    """Validate FORGEMIND_PARSER syntax and required external parser endpoints."""
    ...
    if errors:
        raise ParserRoutingConfigError(
            "Invalid FORGEMIND_PARSER configuration: " + "; ".join(errors)
        )
...
def _rules_enable_smart_heading(rules: str) -> bool:
    """True when a docx-matching native FORGEMIND_PARSER rule sets ``smart_heading=true``.
...
def validate_smart_heading_dependencies(parser_rules: str | None = None) -> None:
    ...
    ``FORGEMIND_PARSER`` rule carries ``native(smart_heading=true)``. Per-file
    ...
    from forgemind.parser.docx.smart_heading.nlp import ensure_spacy_models_installed

    ensure_spacy_models_installed(
        "smart_heading is enabled by DOCX_SMART_HEADING or a FORGEMIND_PARSER rule"
    )
...
def _validate_smart_heading_max_chars() -> None:
    ...
    source :func:`~forgemind.parser.docx.smart_heading.guardrails.validate_heading_max_chars_env`,
...
def _matching_rule_directives(
    ...
    *,
    ...
    """Find the first matching ``FORGEMIND_PARSER`` rule for ``file_path``.
...
def resolve_parser_directives(
    ...
    *,
    ...
    Resolution order (mirrors :func:`resolve_file_parser_engine`):
        ...
        2. ``FORGEMIND_PARSER`` rules — first matching rule provides defaults
...
def resolve_stored_document_parser_engine(
    ...
    if content_data:
        ...
        # All forgemind rows reuse the already-parsed sidecar (sidecar optional;
        # ReuseParser tolerates a missing one). Reached on resume/retry.
        if doc_format == FULL_DOCS_FORMAT_FORGEMIND:
