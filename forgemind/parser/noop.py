...
- :class:`ReuseParser` handles ``forgemind`` rows — a document already parsed
...
from forgemind.constants import FULL_DOCS_FORMAT_FORGEMIND, FULL_DOCS_FORMAT_RAW
from forgemind.parser.base import BaseParser, ParseContext, ParseResult


class ReuseParser(BaseParser):
    """Reuse an already-parsed (``forgemind``-format) document's sidecar."""
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        ...
        doc_format = ctx.content_data.get("parse_format", FULL_DOCS_FORMAT_FORGEMIND)
        merged_text = strip_forgemind_doc_prefix(
