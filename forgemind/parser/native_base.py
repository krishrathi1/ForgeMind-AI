...
``NativeParserBase.parse`` fixes the common local-parse flow once:
    ...
    → persist full_docs (forgemind) → archive source
...
from forgemind.parser.base import BaseParser, ParseContext, ParseResult
...
class NativeParserBase(BaseParser):
    ...
    def _build_llm_submit(
        ...
        from forgemind.utils import get_llm_cache_identity, use_llm_func_with_cache
    ...
    def finalize_parse_warnings(
        ...
        *,
        ...
        :class:`~forgemind.parser.docx.parser.NativeDocxParser`). The base stays
    ...
    async def parse(self, ctx: ParseContext) -> ParseResult:
        from forgemind.parser.routing import decode_parse_engine, encode_parse_engine
        ...
        await ctx.rag._persist_parsed_full_docs(
            ctx.doc_id,
            {
                "content": make_forgemind_doc_content(parsed_data["content"]),
