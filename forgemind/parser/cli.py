...
Invocation::
    ...
    python -m forgemind.parser.cli path/to/sample.docx --engine native
    python -m forgemind.parser.cli path/to/sample.pdf  --engine mineru
    python -m forgemind.parser.cli path/to/sample.pdf  --engine docling --force-reparse
...
def _normalize_direct_script_sys_path() -> None:
    ...
    # Direct execution adds forgemind/parser to sys.path, which makes the
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    from forgemind.parser.registry import supported_parser_engines
...
async def _run(args: argparse.Namespace) -> int:
    ...
    from forgemind.parser.registry import get_parser, suffix_capabilities
    from forgemind.parser.debug import build_debug_rag
    from forgemind.parser.docx.parse_document import DocxContentError
    from forgemind.utils import compute_mdhash_id
    import forgemind.pipeline as pipeline_mod
    import forgemind.utils_pipeline as utils_pipeline_mod
    ...
    with ExitStack() as stack:
        ...
        # ctx.archive_source -> forgemind.pipeline.archive_docx_source_after_...
        stack.enter_context(
            mock.patch.object(
                pipeline_mod,
...
def main(argv: list[str] | None = None) -> int:
    ...
    # so a `forgemind.parsers` entry-point engine is selectable here exactly
