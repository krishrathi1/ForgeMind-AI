"""Third-party parser engine discovery (``forgemind.parsers`` entry points).

A third-party package exposes parser engines by declaring an entry point in
the ``forgemind.parsers`` group::
...
its own :func:`forgemind.parser.registry.register_parser` call(s).  The
...
routing-rule validation so ``FORGEMIND_PARSER`` may reference third-party
engine names) and the debug CLI (``forgemind.parser.cli.main``, before
``--engine`` choices are built).  Library users embedding ForgeMind directly
...
ENTRY_POINT_GROUP = "forgemind.parsers"
...
def load_third_party_parsers(*, force: bool = False) -> list[str]:
    """Discover and run all ``forgemind.parsers`` entry points.
