...
# MAX_HEADING_LENGTH is imported from forgemind.constants above (a raw-char UI
...
def _diagnose_invalid_docx(file_path: str) -> tuple[str, str]:
    ...
    convert_solution = (
        ...
        "  3. Re-upload the converted .docx to ForgeMind"
    )
    ...
    if not head:
        return (
            ...
            "  2. Re-upload a complete copy to ForgeMind",
        )
    ...
    if head.startswith(b"PK\x03\x04") and not zipfile.is_zipfile(file_path):
        ...
        return (
            ...
            "  1. Check the original document opens correctly\n"
            "  2. Re-upload a complete, uncorrupted copy to ForgeMind",
        )
...
def extract_para_id(para_element) -> str:
    ...
    Returns:
        ...
        propagate the ``None`` upward — the ForgeMind adapter counts these
    ...
    *,
    ...
    Args:
        ...
        parse_warnings: Optional out-dict that this function mutates with
            ...
            per table whose every cell lacks one. Callers (the ForgeMind
        ...
        parse_metadata: Optional out-dict that this function mutates with
            ...
            in document order (regardless of level). Used by the ForgeMind
    ...
    from forgemind.parser.docx.smart_heading import guardrails as _guards
