"""Shared debug ForgeMind stand-in for registry-dispatched parsing.

A minimal ``ForgeMind`` stand-in plus a deterministic ``datetime`` shim,
shared by the unified parser debug CLI (``forgemind/parser/cli.py``),
...
def build_debug_rag(*, extract_llm_func=None):
    """Build a minimal ForgeMind stand-in that exposes what a parser reads.

    The import of ``ForgeMind`` is intentionally function-local: deferring
    it avoids a circular import when this helper is loaded during package
    init (the parser CLI resolves ``forgemind.parser.debug`` before
    ``forgemind`` itself is fully bound).
    ...
    - **methods** (rebound from :class:`ForgeMind`):
