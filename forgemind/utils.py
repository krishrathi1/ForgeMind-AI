...
def _patch_ascii_colors_console_handler() -> None:
    ...
    if getattr(ConsoleHandler, "_forgemind_patched", False):
        return
    ...
    ConsoleHandler._forgemind_patched = True  # type: ignore[attr-defined]
...
if TYPE_CHECKING:
    from forgemind.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
PERFORMANCE_TIMING_LOGS = (
    os.getenv("FORGEMIND_PERFORMANCE_TIMING_LOGS", "false").lower() == "true"
)
...
class ForgemindPathFilter(logging.Filter):
    """Filter for forgemind logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        add_filter: Whether to add ForgemindPathFilter to the logger
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to forgemind.log in LOG_DIR or cwd
    ...
    if add_filter:
        path_filter = ForgemindPathFilter()
...
class EmbeddingFunc:
    ...
    Args:
        ...
        max_token_size: Enable embedding token limit checking for description summarization(Set embedding_token_limit in ForgeMind)
...
class VectorStorageConsistencyError(Exception):
    ...
    and query results may be incomplete until it is rebuilt. Stop the ForgeMind
    server and run the offline rebuild tool (``forgemind-rebuild-vdb``) to restore
    consistency.
    """
...
def priority_limit_async_func_call(
    ...
    def final_decro(func):
        ...
        publish_stats = False
        shared = None  # lazily imported forgemind.kg.shared_storage module
        ...
        def _resolve_mode() -> bool:
            ...
            if concurrency_group is None:
                ...
                return True
            if shared is None:
                from forgemind.kg import shared_storage as shared_module
...
def write_json(json_obj, file_name):
    ...
    """
    from forgemind.file_atomic import atomic_write
...
def _normalized_child_offsets(
    ...
    projection :mod:`forgemind.sidecar.backfill` uses.
    """
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# ForgeMind Data Export\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as txtfile:
            txtfile.write("FORGEMIND DATA EXPORT\n")
            txtfile.write("=" * 80 + "\n\n")
...
class TruncatedResponse(str):
    ...
    in ``use_llm_func_with_cache`` and ``forgemind.operate``.
    """
