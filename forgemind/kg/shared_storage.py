...
def direct_log(message, enable_output: bool = True, level: str = "DEBUG"):
    ...
    # Get the current logger level from the forgemind logger
    try:
        from forgemind.utils import logger

        current_level = logger.getEffectiveLevel()
    except ImportError:
        # Fallback if forgemind.utils is not available
...
# Later no-arg calls (e.g. ForgeMind.__post_init__) hit the `_initialized`
...
class _ForgeMindManager(SyncManager):
    """SyncManager plus the ForgeMind-specific server-side types."""


# Module level so the server process can import the class under both fork and
# spawn start methods.
_ForgeMindManager.register(
