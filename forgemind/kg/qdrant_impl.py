...
def _find_legacy_collection(
    ...
    created by older versions of ForgeMind:

    1. forgemind_vdb_{namespace} - if model_suffix is provided (HIGHEST PRIORITY)
    2. {workspace}_{namespace} or {namespace} - no matter if model_suffix is provided or not
    3. forgemind_vdb_{namespace} - fall back value no matter if model_suffix is provided or not (LOWEST PRIORITY)
    ...
    candidates = [
        f"forgemind_vdb_{namespace}" if model_suffix else None,
        f"{workspace}_{namespace}" if workspace else None,
        f"forgemind_vdb_{namespace}",
        namespace,
    ]
...
class QdrantVectorDBStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        ...
        # Example: "forgemind_vdb_chunks_text_embedding_ada_002_1536d"
        # Ensure model_suffix is not empty before appending
        if self.model_suffix:
            self.final_namespace = f"forgemind_vdb_{self.namespace}_{self.model_suffix}"
            logger.info(f"Qdrant collection: {self.final_namespace}")
        else:
            # Fallback: use legacy namespace if model_suffix is unavailable
            self.final_namespace = f"forgemind_vdb_{self.namespace}"
            logger.warning(
