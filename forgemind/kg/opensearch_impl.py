"""
OpenSearch Storage Implementation for ForgeMind

This module provides OpenSearch-based storage backends for ForgeMind,
...
# knobs (forgemind/kg/mongo_impl.py) so behaviour stays consistent across
...
async def _verify_mirrored_id_mapping(client: AsyncOpenSearch, index_name: str) -> None:
    ...
    cross-shard pagination tiebreaker. Indices created by older ForgeMind
    ...
    if "__mirrored_id" not in props:
        raise RuntimeError(
            ...
            f"This index was likely created by an older ForgeMind release. "
...
class OpenSearchKVStorage(BaseKVStorage):
    ...
    async def finalize(self):
        ...
        buffer counts -- otherwise ``ForgeMind.finalize_storages()`` would
...
class OpenSearchDocStatusStorage(DocStatusStorage):
    ...
    async def _ensure_content_hash_mapping(self) -> None:
        ...
        Indices created by older ForgeMind releases lack content_hash entirely.
...
class OpenSearchVectorDBStorage(BaseVectorStorage):
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        flush, which matches the contract used by other ForgeMind storage
    ...
    async def get_by_id(self, id: str) -> dict[str, Any] | None:
        ...
        ForgeMind vector backend (see ``NanoVectorDBStorage.get_by_id``).
