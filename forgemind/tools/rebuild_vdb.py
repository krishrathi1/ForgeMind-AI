#!/usr/bin/env python3
"""
Offline Vector Storage (VDB) Rebuild Tool for ForgeMind

The knowledge graph and the text_chunks KV store are the authoritative data
sources in ForgeMind. If a vector storage write fails at runtime (e.g. during
...
IMPORTANT: Shut down the ForgeMind Server (and any other writers) before
running this tool.

Usage:
    forgemind-rebuild-vdb
    # or
    python -m forgemind.tools.rebuild_vdb

Configuration is read from .env / environment variables, exactly like the
ForgeMind server (FORGEMIND_GRAPH_STORAGE, FORGEMIND_VECTOR_STORAGE,
FORGEMIND_KV_STORAGE, WORKSPACE, WORKING_DIR, EMBEDDING_* ...). The embedding
...
from forgemind.kg import STORAGE_ENV_REQUIREMENTS
...
async def rebuild_entities_vdb(
    ...
    *,
    ...
    """
    from forgemind.operate import _truncate_vdb_content
...
async def enumerate_kv_keys(kv) -> List[str]:
    ...
    scans (same patterns as forgemind/tools/clean_llm_query_cache.py).
    ...
    if storage_name == "PGKVStorage":
        from forgemind.kg.postgres_impl import namespace_to_table_name
...
class RebuildTool:
    ...
    def resolve_storage_names(self) -> Dict[str, str]:
        return {
            "graph": os.getenv("FORGEMIND_GRAPH_STORAGE", "NetworkXStorage"),
            "vector": os.getenv("FORGEMIND_VECTOR_STORAGE", "NanoVectorDBStorage"),
            "kv": os.getenv("FORGEMIND_KV_STORAGE", "JsonKVStorage"),
        }
    ...
    def build_embedding_func(self) -> EmbeddingFunc | None:
        ...
        try:
            ...
            from forgemind.api.forgemind_server import (
                create_embedding_function_from_args,
            )
        except ImportError as e:
            print(f"\n⚠️  Could not import the ForgeMind API package: {e}")
            print('   Rebuild requires the api extra: pip install "forgemind-ai[api]"')
    ...
    def build_global_config(self) -> Dict[str, Any]:
        global_config: Dict[str, Any] = {
            ...
            # Backend selection, mirroring ForgeMind._build_global_config. PG
    ...
    async def setup_storages(self) -> bool:
        ...
        if not self.embedding_available:
            ...
            async def _no_embedding(*_args, **_kwargs):
                raise RuntimeError(
                    ...
                    'Install the api extra: pip install "forgemind-ai[api]"'
                )
        ...
        # Namespaces and meta_fields must match ForgeMind's own storage setup
            ...
            for storage in self.all_storages():
                if storage is not None:
                    try:
                        ...
                        pass
            try:
                from forgemind.kg.shared_storage import finalize_share_data
