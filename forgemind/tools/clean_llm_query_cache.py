#!/usr/bin/env python3
"""
LLM Query Cache Cleanup Tool for ForgeMind
...
Usage:
    python -m forgemind.tools.clean_llm_query_cache
    # or
    python forgemind/tools/clean_llm_query_cache.py
...
from forgemind.kg.shared_storage import set_all_update_flags
...
class CleanupTool:
    ...
    def get_storage_class(self, storage_name: str):
        ...
        if storage_name == "JsonKVStorage":
            ...
            from forgemind.kg.redis_impl import RedisKVStorage

            return RedisKVStorage
        elif storage_name == "PGKVStorage":
            from forgemind.kg.postgres_impl import PGKVStorage

            return PGKVStorage
        elif storage_name == "MongoKVStorage":
            from forgemind.kg.mongo_impl import MongoKVStorage

            return MongoKVStorage
        elif storage_name == "OpenSearchKVStorage":
            from forgemind.kg.opensearch_impl import OpenSearchKVStorage
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("LLM Query Cache Cleanup Tool - ForgeMind")
        print("=" * 60)
    ...
    async def setup_storage(self) -> tuple:
        ...
        if storage_name == "JsonKVStorage":
            ...
            print("  • ForgeMind Server is completely shut down")
            print("  • No other programs are accessing the storage files")
            print("\n" + "=" * 60)

            confirm = (
                input("\nHas ForgeMind Server been shut down? (yes/no): ")
                .strip()
                .lower()
            )
            if confirm != "yes":
                print(
                    "\n✓ Operation cancelled - Please shut down ForgeMind Server first"
                )
                return None, None, None
