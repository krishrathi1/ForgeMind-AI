...
`FORGEMIND_*` storage selectors; never calls the LLM or embedder):

```bash
python -m forgemind.tools.kg_integrity_repair             # report
python -m forgemind.tools.kg_integrity_repair --apply     # repair
python -m forgemind.tools.kg_integrity_repair --verbose   # per-doc details
```
