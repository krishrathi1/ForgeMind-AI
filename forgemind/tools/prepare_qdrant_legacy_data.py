#!/usr/bin/env python3
"""
Qdrant Legacy Data Preparation Tool for ForgeMind
...
New Collections (with workspace_id):
    ...
    - forgemind_vdb_entities
    - forgemind_vdb_relationships
...
Usage:
    python -m forgemind.tools.prepare_qdrant_legacy_data
    # or
    python forgemind/tools/prepare_qdrant_legacy_data.py

    # Specify custom workspace
    python -m forgemind.tools.prepare_qdrant_legacy_data --workspace space1

    # Process specific collection types only
    python -m forgemind.tools.prepare_qdrant_legacy_data --types chunks,entities

    # Dry run (preview only, no actual changes)
    python -m forgemind.tools.prepare_qdrant_legacy_data --dry-run
"""
...
COLLECTION_NAMESPACES = {
    "chunks": {
        "new": "forgemind_vdb_chunks",
        "suffix": "chunks",
    },
    "entities": {
        "new": "forgemind_vdb_entities",
        "suffix": "entities",
    },
    "relationships": {
        "new": "forgemind_vdb_relationships",
...
class QdrantLegacyDataPreparationTool:
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("Qdrant Legacy Data Preparation Tool - ForgeMind")
        print("=" * 60)
        if self.dry_run:
