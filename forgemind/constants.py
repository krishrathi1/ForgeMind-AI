"""
Centralized configuration constants for ForgeMind.

This module defines default values for configuration constants used across
different parts of the ForgeMind system. Centralizing these values ensures
...
# entity_name / src_id / tgt_id fields in forgemind/kg/milvus_impl.py.
...
# by forgemind/parser/docx/smart_heading (same live-env pattern as the
...
# enqueue docs_format (the 'forgemind' ingestion entrypoint was removed).
FULL_DOCS_FORMAT_FORGEMIND = "forgemind"  # content in ForgeMind Document files
FULL_DOCS_FORMAT_PENDING_PARSE = (
    "pending_parse"  # file saved but not yet parsed; parse_native will read from disk
)
# Marker prefix for full_docs.content when format=forgemind.
...
# preview without loading the full ForgeMind Document file.
FORGEMIND_DOC_CONTENT_PREFIX = "{{LRdoc}}"
# Engine identifier strings (registry keys). The set of user-selectable
# engines and their suffix capabilities now live in
# forgemind.parser.registry (ParserSpec table) — the single source of truth.
...
# Per-file processing options carried by filename hints / FORGEMIND_PARSER rules.
