...
``addon_params`` is a free-form configuration dict on :class:`ForgeMind` that
...
- :class:`ObservableAddonParams` — a ``dict`` subclass that calls a callback
  whenever the contents change so the ForgeMind runtime can invalidate cached
  derived state.
...
from forgemind.constants import DEFAULT_SUMMARY_LANGUAGE
...
def default_addon_params() -> dict[str, Any]:
    ...
    # module via ``ForgeMind`` construction paths.
