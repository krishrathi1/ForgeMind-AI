...
from forgemind.chunker import chunking_by_token_size
...
from forgemind.types import KnowledgeGraph
from dotenv import load_dotenv
from forgemind.pipeline import _PipelineMixin
from forgemind.kg.factory import get_storage_class
from forgemind.addon_params import (
    ObservableAddonParams,
    normalize_addon_params,
)
from forgemind.llm_roles import (
    ROLE_NAMES,
    ROLES,
    RoleLLMConfig,
    RoleSpec,  # noqa: F401  # re-exported via forgemind/__init__.py
    _optional_env_int,
    _RoleLLMMixin,
    _RoleLLMState,
)
from forgemind.storage_migrations import _StorageMigrationMixin
...
def _run_sync(
    ...
    * **A different — but still alive — loop than the storages bound to**
      ...
      ``asyncio.Lock`` objects in ``forgemind.kg.shared_storage`` are bound to
    ...
    *closed* ``owning_loop`` means the loop the storages were initialized on is
    ...
    ``ForgeMind`` instance; callers that must invoke them from multiple threads
    ...
    Args:
        ...
        owning_loop: The loop the instance's storages were initialized on
            (``ForgeMind._owning_loop``). ``None`` when storages have not been
    ...
    if (
        ...
        raise RuntimeError(
            f"{sync_name}() must run on the same event loop this ForgeMind "
            ...
            f"thread. ForgeMind's shared storage locks are bound to the original "
...
@final
@dataclass
class ForgeMind(_RoleLLMMixin, _StorageMigrationMixin, _PipelineMixin):
    """ForgeMind: Simple and Fast Retrieval-Augmented Generation."""
    ...
    :func:`forgemind.parser.routing.default_chunker_config`.  Per-doc
    ...
    **When this function is actually invoked.** The chunker dispatch in
    ...
    document's ``process_options``:
      ...
      - If ``process_options`` explicitly contains a chunking selector
        ...
        :mod:`forgemind.chunker` (``chunking_by_fixed_token`` for ``F``,
    ...
    :attr:`forgemind.parser.routing.ProcessOptions.chunking_explicit`.

    **Signature** — preserved unchanged from earlier ForgeMind releases
    ...
    Defaults to :func:`forgemind.chunker.chunking_by_token_size`.
    """
    ...
    # TODO: Deprecated (ForgeMind will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, forgemind will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    def _apply_chunk_size_overlay(self) -> None:
        ...
        2. Strategy-specific env (``CHUNK_F_SIZE`` / ``CHUNK_R_SIZE`` /
           ...
           :func:`forgemind.parser.routing.default_chunker_config` when it
        ...
        3. Legacy constructor field
           (``ForgeMind(chunk_token_size=…, chunk_overlap_token_size=…)``).
    ...
    def __post_init__(self, addon_params: dict[str, Any] | None):
        ...
        logger.debug(f"ForgeMind init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple ForgeMind instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            if default_workspace is None:
                ...
                logger.info(
                    f"Creating ForgeMind instance with workspace='{self.workspace}' "
                    f"while default workspace is set to '{default_workspace}'"
                )

            # Auto-initialize pipeline_status for this workspace
            from forgemind.kg.shared_storage import initialize_pipeline_status
    ...
    async def ainsert(
        ...
        The ForgeMind **server / REST API does not call this method** — it
    ...
    async def ainsert_custom_kg(
        ...
        .. warning:: (issue #3400 Phase 5 — direct-writer audit)
           ...
           offline ``forgemind.tools.kg_integrity_repair`` audit can
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        from forgemind.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        from forgemind.utils_graph import adelete_by_relation
    ...
    async def get_relation_info(
        ...
        """
        from forgemind.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        from forgemind.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        from forgemind.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        from forgemind.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        from forgemind.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        from forgemind.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from forgemind.utils import aexport_data as utils_aexport_data
...
# passed through ForgeMind(addon_params=...). InitVars are not stored as
...
ForgeMind.addon_params = property(  # type: ignore[attr-defined]
    ForgeMind._get_addon_params,
    ForgeMind._set_runtime_addon_params,
)
