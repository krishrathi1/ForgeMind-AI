...
received. When a document was parsed into the ForgeMind sidecar format
...
:func:`forgemind.sidecar.writer.write_sidecar` and
:func:`forgemind.utils_pipeline.load_forgemind_document_content` build it as
...
from forgemind.chunk_schema import normalize_chunk_sidecar
...
# forgemind/sidecar/writer.py and forgemind/utils_pipeline.py.
_BLOCK_SEPARATOR = "\n\n"
...
def _load_content_blocks(blocks_path: str) -> list[tuple[str, str]]:
    ...
    Note: ``utils_pipeline.load_forgemind_document_content`` (which produces the
...
def _build_block_spans(
    ...
    ``utils_pipeline.load_forgemind_document_content``). Returns ``(merged, spans)``
