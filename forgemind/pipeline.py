"""Document ingestion pipeline mixin for the ForgeMind class.

This module isolates the document parse/enqueue/extraction pipeline so that
``forgemind.py`` stays focused on storage management, querying, and editing.
The mixin is wired into :class:`forgemind.ForgeMind` via multiple inheritance
...
from forgemind.exceptions import (
...
from forgemind.parser.llm_bridge import LLMBridgePipelineCancelled
from forgemind.parser.registry import (
    get_parser,
...
from forgemind.utils_pipeline import (
    ...
    # directly): the parser layer resolves these as ``forgemind.pipeline.<name>``
...
class _PipelineMixin:
    """Mixin providing document ingestion pipeline methods for ForgeMind.

    Designed to be combined as a base of ForgeMind only.  Relies on
    ForgeMind-provided attributes (``self.full_docs``, ``self.doc_status``,
    ...
    async def apipeline_enqueue_documents(
        ...
        Args:
            ...
            ids: list of unique document IDs, if not provided, MD5 hash IDs will be generated (from content or file_path).
                **Providing ``ids`` marks the SDK raw direct-insert path**
                (:meth:`ForgeMind.ainsert`) and takes precedence over
            ...
            chunk_options: per-document chunker parameter snapshot.
                ...
                :meth:`ForgeMind.ainsert` — should call
        ...
        if docs_format not in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_PENDING_PARSE):
            raise ValueError(
                ...
                "The 'forgemind' enqueue format was removed; already-parsed "
        ...
        def _process_options_at(index: int) -> str:
            if process_options is None:
                return ""
            from forgemind.parser.routing import sanitize_process_options
        ...
        def _chunk_options_at(index: int) -> dict[str, Any]:
            ...
            F-strategy runtime args (``split_by_character`` /
            ``split_by_character_only`` from :meth:`ForgeMind.ainsert`)
            are baked into the snapshot upstream — ainsert calls
            :func:`forgemind.parser.routing.resolve_chunk_options` itself
        ...
        # 1. Validate ids and build contents (when forgemind: no content dedup, content may be empty)
        if ids is not None:
    ...
    async def _run_pipeline_batch(
        ...
        *,
        ...
        def _group_concurrency(group: str) -> int:
            # Built-in groups keep their existing ForgeMind fields (env +
    ...
    async def _parse_worker(
        ...
        while True:
            ...
            try:
                ...
                # reuse/passthrough (raw/forgemind/unknown_source) are skipped.
                if (
                ...
                # has already persisted the parsed body to full_docs (forgemind /
    ...
    async def _analyze_worker(self, ctx: _BatchRunContext) -> None:
        ...
        body (pending_parse → forgemind / raw documents start with empty
        ...
        *,
        ...
        async with ctx.semaphore:
            try:
                ...
                # strip the forgemind marker according to the stored parse_format
                # — parse persisted the body for every engine before enqueue.
                content = strip_forgemind_doc_prefix(
                ...
                if doc_process_opts.chunking_explicit:
                    ...
                    # keeps ``ForgeMind.ainsert`` — which intentionally does NOT
                ...
                # parse_format=raw, which silently mislabelled forgemind docs;
    ...
    async def _purge_stale_extraction_if_resuming(
        ...
        *,
        ...
        content_already_extracted = isinstance(content_data, dict) and (
            (
                content_data.get("parse_format") == FULL_DOCS_FORMAT_FORGEMIND
        ...
        from forgemind.parser.routing import normalize_parser_engine
        ...
        if intended_engine and stored_engine and intended_engine != stored_engine:
            log_message = (
                ...
                f"FORGEMIND_PARSER implies engine="
    ...
    async def _persist_parsed_full_docs(
        ...
        # for forgemind-format) so cross-filename dedup fires regardless of
        # whether the same body was ingested as raw text or via a sidecar.
        # ``strip_forgemind_doc_prefix`` is a no-op for non-forgemind formats.
        if fmt in (FULL_DOCS_FORMAT_RAW, FULL_DOCS_FORMAT_FORGEMIND):
            content_hash = compute_text_content_hash(
                strip_forgemind_doc_prefix(record.get("content") or "", fmt)
            )
    ...
    async def analyze_multimodal(
        ...
        *,
        ...
        """Phase 2: Multimodal analysis (VLM). Writes llm_analyze_result to ForgeMind Document.
        ...
        # Docling, and pre-existing ForgeMind documents reused from disk
        ...
        if enabled_modalities and tokenizer is not None:
            try:
                from forgemind.multimodal_context import (
        ...
        try:
            ...
            from forgemind.llm._vision_utils import (
                image_audit_metadata,
                image_cache_metadata,
                normalize_image_inputs,
                read_image_dimensions,
            )
            from forgemind.prompt_multimodal import (
                IMAGE_TYPE_ENUM,
                IMAGE_TYPE_FALLBACK,
                MULTIMODAL_PROMPTS,
            ...
            # in forgemind.utils).  When the flag is off we must NOT save the
    ...
    def _build_mm_chunks_from_sidecars(
        ...
        def _render(
            ...
            # ``forgemind.operate._parse_mm_display_name`` (regex
