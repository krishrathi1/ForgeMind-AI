...
from forgemind.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to forgemind.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to forgemind.log
...
def on_starting(server):
    ...
    # Log the location of the ForgeMind log file
    print(f"ForgeMind log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("forgemind", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up forgemind submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("forgemind."):
