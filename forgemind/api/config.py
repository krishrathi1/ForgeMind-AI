...
from forgemind import ROLES
from forgemind.utils import get_env_value, logger
from forgemind.llm.binding_options import (
    BedrockLLMOptions,
...
from forgemind.base import OllamaServerInfos
import sys
...
DEFAULT_TOKEN_SECRET = "forgemind-jwt-default-secret-key!"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="ForgeMind API Server")
    ...
    parser.add_argument(
        "--key",
        type=str,
        default=get_env_value("FORGEMIND_API_KEY", None),
        help="API key for authentication. This protects forgemind server against unauthorized access",
    )
    ...
    parser.add_argument(
        ...
        help="Name for the simulated Ollama model (default: from env or forgemind)",
    )
    ...
    parser.add_argument(
        "--api-prefix",
        type=str,
        default=get_env_value("FORGEMIND_API_PREFIX", ""),
    ...
    args.kv_storage = get_env_value(
        "FORGEMIND_KV_STORAGE", DefaultRAGStorageConfig.KV_STORAGE
    )
    args.doc_status_storage = get_env_value(
        "FORGEMIND_DOC_STATUS_STORAGE", DefaultRAGStorageConfig.DOC_STATUS_STORAGE
    )
    args.graph_storage = get_env_value(
        "FORGEMIND_GRAPH_STORAGE", DefaultRAGStorageConfig.GRAPH_STORAGE
    )
    args.vector_storage = get_env_value(
        "FORGEMIND_VECTOR_STORAGE", DefaultRAGStorageConfig.VECTOR_STORAGE
    )
    ...
    # --- Per-role LLM configuration (driven by forgemind.ROLES registry) ---
    for spec in ROLES:
    ...
    ollama_server_infos.FORGEMIND_NAME = args.simulated_model_name
    ollama_server_infos.FORGEMIND_TAG = args.simulated_model_tag
...
def initialize_config(args=None, force=False):
    ...
    which is useful for programmatic usage, testing, or embedding ForgeMind
