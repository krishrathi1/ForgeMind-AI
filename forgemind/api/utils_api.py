"""
Utility functions for the ForgeMind API.
"""
...
from forgemind.api.runtime_validation import validate_runtime_target_from_env_file
...
def display_splash_screen(args: argparse.Namespace) -> None:
    """
    Display a colorful splash screen showing ForgeMind server configuration
    ...
    line1_text = f"ForgeMind Server v{core_version}/{api_version}"
    ...
    ASCIIColors.yellow(f"{ollama_server_infos.FORGEMIND_MODEL}")
    ...
    # neither AUTH_ACCOUNTS nor FORGEMIND_API_KEY set, all requests are allowed).
    ...
    reachable from this machine. Set FORGEMIND_API_KEY, or AUTH_ACCOUNTS together
    ...
    Secure the server before exposing it to a network by setting at least one of:
      - FORGEMIND_API_KEY=<a-strong-secret>   (X-API-Key header authentication)
