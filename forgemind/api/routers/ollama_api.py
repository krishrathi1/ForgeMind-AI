...
from forgemind.constants import DEFAULT_QUERY_PRIORITY
from forgemind.utils import TiktokenTokenizer
from forgemind.api.utils_api import get_combined_auth_dependency, internal_server_error
from fastapi import Depends


# query mode according to query prefix (bypass is not ForgeMind quer mode)
...
class OllamaAPI:
    def __init__(self, rag: ForgeMind, top_k: int = 60, api_key: Optional[str] = None):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.FORGEMIND_MODEL,
                        "model": self.ollama_server_infos.FORGEMIND_MODEL,
                        "modified_at": self.ollama_server_infos.FORGEMIND_CREATED_AT,
                        "size": self.ollama_server_infos.FORGEMIND_SIZE,
                        "digest": self.ollama_server_infos.FORGEMIND_DIGEST,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.FORGEMIND_NAME,
                            "families": [self.ollama_server_infos.FORGEMIND_NAME],
                            "parameter_size": "13B",
                ...
                models=[
                    {
                        ...
                        "size_vram": self.ollama_server_infos.FORGEMIND_SIZE,
                    }
                ]
            )
        ...
        async def generate(raw_request: Request):
            ...
            For compatibility purpose, the request is not processed by ForgeMind,
                ...
                if request.stream:
                    ...
                    async def stream_generator():
                        ...
                        if isinstance(response, str):
                            ...
                            data = {
                                "model": self.ollama_server_infos.FORGEMIND_MODEL,
                                "created_at": self.ollama_server_infos.FORGEMIND_CREATED_AT,
                                "response": response,
                                "done": False,
                            }
        ...
        async def chat(raw_request: Request):
            ...
            Routes user queries through ForgeMind by selecting query mode based on query prefix.
