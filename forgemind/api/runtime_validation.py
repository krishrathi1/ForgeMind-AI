...
def load_runtime_target_from_env_file(env_path: str | Path = ".env") -> str | None:
    """Return the raw FORGEMIND_RUNTIME_TARGET value from the `.env` file, if present."""

    env_values = dotenv_values(str(env_path))
    runtime_target = env_values.get("FORGEMIND_RUNTIME_TARGET")
...
def validate_runtime_target(
    ...
    if normalized_target == "host":
        if runtime_environment.in_container:
            return (
                False,
                "Configuration error in .env: FORGEMIND_RUNTIME_TARGET=host.\n"
    ...
    if normalized_target in _CONTAINER_RUNTIME_TARGETS:
        ...
        return (
            False,
            f"Configuration error in .env: FORGEMIND_RUNTIME_TARGET={runtime_target}.\n"
    ...
    return (
        False,
        f"Configuration error in .env: FORGEMIND_RUNTIME_TARGET={runtime_target!r}.\n"
...
def validate_runtime_target_from_env_file(
    ...
    """Load FORGEMIND_RUNTIME_TARGET from `.env` and validate it if declared."""
