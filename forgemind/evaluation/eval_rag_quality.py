#!/usr/bin/env python3
"""
RAGAS Evaluation Script for ForgeMind System
...
Usage:
    ...
    python forgemind/evaluation/eval_rag_quality.py -d my_test.json

    # Specify custom RAG endpoint
    python forgemind/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
    python forgemind/evaluation/eval_rag_quality.py -r http://my-server.com:9621
...
Results are saved to: forgemind/evaluation/results/
...
class RAGEvaluator:
    ...
    def __init__(self, test_dataset_path: str = None, rag_api_url: str = None):
        ...
        Args:
            ...
            rag_api_url: Base URL of ForgeMind API (e.g., http://localhost:9621)
        ...
        if rag_api_url is None:
            rag_api_url = os.getenv("FORGEMIND_API_URL", "http://localhost:9621")
    ...
    def _display_configuration(self):
        ...
        logger.info("  • ForgeMind API:         %s", self.rag_api_url)
    ...
    async def generate_rag_response(
        ...
        """
        Generate RAG response by calling ForgeMind API.
        ...
        Raises:
            Exception: If ForgeMind API is unavailable.
        """
        try:
            payload = {
                "query": question,
            ...
            api_key = os.getenv("FORGEMIND_API_KEY")
            ...
            raise Exception(
                f"❌ Cannot connect to ForgeMind API at {self.rag_api_url}\n"
                f"   Make sure ForgeMind server is running:\n"
                f"   python -m forgemind.api.forgemind_server\n"
                f"   Error: {str(e)}"
            )
        except httpx.HTTPStatusError as e:
            raise Exception(
                f"ForgeMind API error {e.response.status_code}: {e.response.text}"
            )
    ...
    async def evaluate_responses(self) -> List[Dict[str, Any]]:
        ...
        logger.info("%s", "=" * 70)
        logger.info("🚀 Starting RAGAS Evaluation of ForgeMind System")
...
async def main():
    ...
    Command-line arguments:
        ...
        --ragendpoint, -r: ForgeMind API endpoint URL (default: http://localhost:9621 or $FORGEMIND_API_URL)
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="RAGAS Evaluation Script for ForgeMind System",
...
Examples:
  ...
  python forgemind/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="ForgeMind API endpoint URL (default: http://localhost:9621 or $FORGEMIND_API_URL environment variable)",
        )

        args = parser.parse_args()

        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Using Real ForgeMind API")
        logger.info("%s", "=" * 70)
