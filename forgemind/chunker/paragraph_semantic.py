"""Paragraph Semantic Chunking for ForgeMind.

Reads a ForgeMind ``.blocks.jsonl`` sidecar — produced by any sidecar-emitting
parser (native / mineru / docling): heading-driven blocks, tables kept whole —
and produces a chunk list compatible with
:func:`forgemind.chunker.chunking_by_token_size`.
...
from forgemind.table_markup import (
...
from forgemind.utils import Tokenizer, logger


# ---------------------------------------------------------------------------
# Threshold ratios — derived from the audit-mode constants in
# forgemind/parser/docx/parse_document.py so the trade-off curves
...
# imported from :mod:`forgemind.table_markup` so the surrounding-context
...
# Mirrors ``_MD_HEADING_RE`` in ``forgemind/parser/_markdown.py`` (the same
...
def _tables_sidecar_path(blocks_path: str) -> str | None:
    ...
    Mirrors :func:`forgemind.utils_pipeline.sidecar_modality_path` naming
...
def _character_split_text(
    ...
    *,
    ...
    """
    from forgemind.chunker.recursive_character import (
...
def _split_long_block(
    ...
    *,
    ...
    Mirrors :func:`forgemind.parser.docx.parse_document.split_long_block`,
...
def _is_heading_only(block: dict[str, Any]) -> bool:
    ...
    accepted ambiguity documented in ``forgemind/parser/_markdown.py`` and is
...
def _merge_small_blocks(
    ...
    *,
    ...
    :func:`forgemind.parser.docx.parse_document.merge_small_blocks`,
...
def chunking_by_paragraph_semantic(
    ...
    *,
    ...
    Signature follows the ForgeMind chunker contract — the standard
    ...
    Args:
        tokenizer: ForgeMind tokenizer (used for all token counting; matches
