...
The chunker exposed here is ``async`` because ForgeMind's
...
Caveats:
  - SemanticChunker does NOT enforce a maximum chunk size; the caller's
    ...
    :func:`forgemind.utils.enforce_chunk_token_limit_before_embedding`.
  - When ``embedding_func`` is ``None`` we log a warning and fall back to
    :func:`forgemind.chunker.chunking_by_recursive_character` — V's only
...
from forgemind.utils import EmbeddingFunc, Tokenizer, logger
...
class _AsyncEmbeddingFuncAdapter(Embeddings):
    """Bridge a ForgeMind :class:`EmbeddingFunc` (async) to LangChain's
...
async def chunking_by_semantic_vector(
    ...
    *,
    ...
    Args:
        tokenizer: ForgeMind tokenizer (used for output token counts).
        ...
        embedding_func: ForgeMind :class:`EmbeddingFunc`. When ``None``
