"""Storage data migration helpers for :class:`ForgeMind`.

Mixed into ForgeMind and runs once at startup (``initialize_storages`` →
...
from forgemind.kg.shared_storage import get_data_init_lock
from forgemind.utils import logger, make_relation_chunk_key


class _StorageMigrationMixin:
    """Mixin that owns one-shot data migrations on :class:`ForgeMind`.

    Mixed into ForgeMind only. Relies on attributes that the main class
