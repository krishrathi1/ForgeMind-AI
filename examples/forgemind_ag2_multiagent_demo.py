"""ForgeMind + AG2 Multi-Agent Demo.

Demonstrates how AG2 agents can use ForgeMind's knowledge graph retrieval
as a tool. Multiple specialized agents collaborate to answer complex
questions over indexed documents.

Architecture:
    User -> AG2 GroupChat (Researcher + Analyst + Writer) -> ForgeMind queries
    - Researcher: uses ForgeMind hybrid search to gather facts
    - Analyst: uses ForgeMind naive (vector) search for complementary results
    - Writer: synthesizes findings into a final answer

Requires:
    pip install forgemind-ai "ag2[openai]>=0.11.4,<1.0"
    export OPENAI_API_KEY="..."

Usage:
    python examples/forgemind_ag2_multiagent_demo.py
"""
...
ForgeMind implements this approach with dual-level retrieval: local search focuses
...
async def setup_forgemind() -> ForgeMind:
    """Initialize ForgeMind and index sample documents."""
    ...
    print("ForgeMind initialized and documents indexed.\n")
    return rag


# --- Async Bridge ---
# AG2 runs tools in a background thread without an event loop.
# We maintain a dedicated event loop in a separate thread for ForgeMind async calls.
...
# Global reference to ForgeMind instance (set in main)
_rag_instance: ForgeMind = None


def create_agents():
    """Create AG2 agents with ForgeMind tools."""
    ...
    researcher = AssistantAgent(
        name="Researcher",
        system_message=(
            "You are a research specialist. Use the forgemind_query tool to search "
    ...
    analyst = AssistantAgent(
        ...
        system_message=(
            ...
            "the forgemind_query tool with mode='naive' to run a direct vector search. "
    ...
    # --- Register ForgeMind as a tool ---

    @user_proxy.register_for_execution()
    @researcher.register_for_llm(
        description=(
            "Query the ForgeMind knowledge base. "
    ...
    def forgemind_query(query: str, mode: str = "hybrid") -> str:
        """Query ForgeMind synchronously (wraps async call)."""
...
def run_multiagent_query(user_proxy, researcher, analyst, writer, question: str):
    """Run a multi-agent GroupChat to answer a question using ForgeMind."""
...
def main():
    ...
    # Start a background event loop for ForgeMind async calls.
    ...
    try:
        # Step 1: Set up ForgeMind (async, runs on the background loop)
        _rag_instance = _run_async(setup_forgemind())

        # Step 2: Create AG2 agents with ForgeMind tools
