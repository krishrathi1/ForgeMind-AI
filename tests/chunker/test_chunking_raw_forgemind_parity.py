"""F-chunking parity between raw and forgemind formats.

After the F-chunking unification, ``apipeline_process_enqueue_documents``
strips the ``{{LRdoc}}`` marker from forgemind-format content and feeds the
...
* T1: identical input text produces identical chunking inputs whether it
  arrives as raw or as an already-parsed forgemind full_docs row pulled
  back through the parse queue on resume (ReuseParser) — the only
  production path for forgemind rows now that the ``docs_format="forgemind"``
...
* T3: ``extraction_meta["parse_format"]`` (surfaced via
  ``doc_status.metadata``) is ``"forgemind"`` for natively-parsed docs —
...
* T6: a ``pending_parse`` document that resolves to forgemind at parse
...
* T7: a raw document whose body *literally* starts with ``{{LRdoc}}``
  ...
  unconditional ``strip_forgemind_doc_prefix`` at the chunking boundary
...
def _attach_chunking_spy(rag: ForgeMind) -> dict:
...
# ---------------------------------------------------------------------------
# T1 — parity: raw vs forgemind produce identical chunking input
# ---------------------------------------------------------------------------
...
@pytest.mark.offline
def test_chunking_input_parity_raw_vs_forgemind(tmp_path):
    """Same body text must reach ``chunking_func`` with byte-identical input
    whether it arrives as raw or as a resumed forgemind full_docs row."""
    ...
    async def _run():
        ...
        assert spy_lr["calls"] >= 1, "forgemind doc never reached chunking_func"
        assert spy_lr["input"] == spy_raw["input"] == expected_merged, (
            "chunking_func received different inputs for raw vs forgemind; "
            f"raw={spy_raw['input']!r}\nlr={spy_lr['input']!r}"
        )
        assert not spy_lr["input"].startswith(FORGEMIND_DOC_CONTENT_PREFIX), (
...
def _stub_docx_blocks(monkeypatch, body_paragraphs: list[str]) -> None:
    ...
    persists the forgemind-format full_docs row."""
...
def test_full_docs_content_carries_full_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert full_doc["content"] == FORGEMIND_DOC_CONTENT_PREFIX + body
            assert full_doc.get("parse_format") == FULL_DOCS_FORMAT_FORGEMIND
            assert full_doc.get("sidecar_location"), (
                "native parse must record the sidecar_location on the "
                "forgemind full_docs row"
            )
            ...
            assert not summary.startswith(FORGEMIND_DOC_CONTENT_PREFIX)
        finally:
...
def test_extraction_meta_records_forgemind_parse_format(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        try:
            ...
            assert metadata.get("parse_format") == FULL_DOCS_FORMAT_FORGEMIND, (
                f"doc_status.metadata.parse_format="
                f"{metadata.get('parse_format')!r}; "
                f"expected {FULL_DOCS_FORMAT_FORGEMIND!r} so the multimodal "
...
class _ListHandler(logging.Handler):
    ...
    The ``forgemind`` logger has ``propagate = False`` so pytest's caplog
...
def test_explicit_R_dispatches_to_recursive_character(tmp_path, monkeypatch):
    ...
    from forgemind.chunker import chunking_by_recursive_character as real_r
    ...
    # The dispatcher does ``from forgemind.chunker import …`` inside the
    ...
    async def _run():
        ...
        forgemind_logger.addHandler(list_handler)
        try:
            await rag.apipeline_enqueue_documents(
            ...
            forgemind_logger.removeHandler(list_handler)
            await rag.finalize_storages()
...
def test_explicit_V_dispatches_to_semantic_vector(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        assert captured.get("embedding_func") is rag.embedding_func, (
            "dispatcher must hand the ForgeMind embedding_func to the V chunker"
        )
        assert legacy_spy["calls"] == 0, (
...
# ---------------------------------------------------------------------------
# T6 — pending_parse → forgemind summary is populated after PROCESSED
# ---------------------------------------------------------------------------


@pytest.mark.offline
def test_pending_parse_forgemind_summary_populated_after_processed(
    ...
    async def _run():
        ...
        try:
            ...
            assert not final_summary.startswith(FORGEMIND_DOC_CONTENT_PREFIX), (
...
def test_raw_text_starting_with_marker_chunked_verbatim(tmp_path):
    ...
    characters.  ``strip_forgemind_doc_prefix`` is a forgemind-only contract
    ...
    body_with_marker = FORGEMIND_DOC_CONTENT_PREFIX + (
    ...
    async def _run():
        ...
        assert spy["input"].startswith(FORGEMIND_DOC_CONTENT_PREFIX), (
