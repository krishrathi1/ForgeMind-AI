...
prompt_with_default() {{
  case "$1" in
    ...
    "PostgreSQL user") printf 'forgemind' ;;
    "PostgreSQL database") printf 'forgemind' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    ...
    assert values["POSTGRES_DATABASE"] == "forgemind"
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL user[rag]|secret:PostgreSQL password: [rag]|PostgreSQL database[forgemind]"
    )
...
""")
    ...
    assert (
        values["PROMPT_LOG"]
        == "PostgreSQL host[localhost]|PostgreSQL port[5432]|PostgreSQL user[rag]|PostgreSQL password: [rag]|PostgreSQL database[forgemind]"
    )
...
prompt_with_default() {{
  case "$1" in
    ...
    "MongoDB database") printf 'ForgeMind' ;;
    "Milvus database name") printf 'forgemind' ;;
    *) printf '%s' "$2" ;;
  esac
}}
...
""")
    assert values["MILVUS_DB_NAME"] == "forgemind"
...
ENV_VALUES[FORGEMIND_VECTOR_STORAGE]="MongoVectorDBStorage"
ENV_VALUES[MONGO_URI]="mongodb://atlas-local.example.com:27017/ForgeMind?replicaSet=rs0&directConnection=true"
...
""")
    assert (
        values["MONGO_URI"]
        == "mongodb://atlas-local.example.com:27017/ForgeMind?replicaSet=rs0&directConnection=true"
    )
    assert values["COMPOSE_MONGO_URI"] == ""
...
""")
    ...
    assert values["FORGEMIND_API_KEY_SET"] == ""
    ...
    assert not any(line.startswith("FORGEMIND_API_KEY=") for line in generated_lines)
