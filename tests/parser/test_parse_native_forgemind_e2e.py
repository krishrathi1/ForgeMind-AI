"""End-to-end test: native docx → ForgeMind Document → stable cache key.
...
``full_docs`` is in FORGEMIND format. ``_load_forgemind_document_content``
...
from forgemind.utils import Tokenizer, TokenizerInterface, compute_args_hash
...
@pytest.mark.offline
def test_native_forgemind_path_produces_stable_merged_text(tmp_path, monkeypatch):
    ...
    async def _run():
        ...
        # ForgeMind-specific writing — that is what we want under test.
        stable_blocks = [
            _block(
        ...
        assert record["parse_format"] == "forgemind"
...
@pytest.mark.offline
def test_native_forgemind_path_writes_blocks_jsonl_and_skips_meta_on_load(
    tmp_path, monkeypatch
):
    """Sanity check: ``_load_forgemind_document_content`` must skip the
...
def test_native_forgemind_path_leaves_unknown_table_caption_empty(
    ...
    async def _run():
        ...
        from forgemind.multimodal_context import enrich_sidecars_with_surrounding
...
@pytest.mark.offline
def test_native_forgemind_path_writes_image_assets_to_blocks_assets_dir(
    tmp_path, monkeypatch
):
