...
/**
 * Inject `<script>window.__FORGEMIND_CONFIG__ = ...</script>` into index.html.
 *
 * This mirrors what the FastAPI server does at request time in production
 * (see `SmartStaticFiles._inject_runtime_config` in
 * `forgemind/api/forgemind_server.py`). Doing it in dev too means the SPA
 ...
 */
function forgemindRuntimeConfigPlugin(env: Record<string, string>): Plugin {
  ...
  const snippet = `<script>window.__FORGEMIND_CONFIG__ = ${payload};</script>`

  return {
    name: 'forgemind-dev-runtime-config',
    apply: 'serve',
    transformIndexHtml(html: string) {
      return html.replace('<!-- __FORGEMIND_RUNTIME_CONFIG__ -->', snippet)
    }
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  // running with FORGEMIND_API_PREFIX=/site01.
  const devApiPrefix = normalizeApiPrefix(env.VITE_DEV_API_PREFIX)

  return {
    plugins: [react(), tailwindcss(), forgemindRuntimeConfigPlugin(env)],
    resolve: {
      alias: {
    ...
    base: './',
    build: {
      outDir: path.resolve(__dirname, '../forgemind/api/webui'),
      emptyOutDir: true,
