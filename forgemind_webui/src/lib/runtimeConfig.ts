 ...
 * REQUEST time: the FastAPI server replaces a `<!-- __FORGEMIND_RUNTIME_CONFIG__ -->`
 * comment in `index.html` with a `<script>window.__FORGEMIND_CONFIG__ = ...</script>`
 * snippet built from `FORGEMIND_API_PREFIX` / `FORGEMIND_WEBUI_PATH`. This module
...
const config =
  (typeof window !== 'undefined' && window.__FORGEMIND_CONFIG__) || {}
