...
import { checkHealth, ForgemindStatus } from '@/api/forgemind'
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null; lastTokenRenewal: string | null; tokenExpiresAt: number | null } => {
  const token = localStorage.getItem('FORGEMIND-API-TOKEN');
  const coreVersion = localStorage.getItem('FORGEMIND-CORE-VERSION');
  const apiVersion = localStorage.getItem('FORGEMIND-API-VERSION');
  const webuiTitle = localStorage.getItem('FORGEMIND-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('FORGEMIND-WEBUI-DESCRIPTION');
  const lastTokenRenewal = localStorage.getItem('FORGEMIND-LAST-TOKEN-RENEWAL');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('FORGEMIND-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('FORGEMIND-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('FORGEMIND-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('FORGEMIND-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('FORGEMIND-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('FORGEMIND-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('FORGEMIND-WEBUI-DESCRIPTION');
      }
      ...
      localStorage.setItem('FORGEMIND-LAST-TOKEN-RENEWAL', formattedTime);
    ...
    logout: () => {
      localStorage.removeItem('FORGEMIND-API-TOKEN');
      localStorage.removeItem('FORGEMIND-LAST-TOKEN-RENEWAL');
