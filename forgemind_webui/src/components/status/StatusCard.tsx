import type {
  ForgemindQueueStatus,
  ForgemindRoleLLMConfig,
  ForgemindStatus
} from '@/api/forgemind'
...
type RoleLLMRow = {
  role: string
  config: ForgemindRoleLLMConfig
  queue?: ForgemindQueueStatus
}
...
type MinerUStatus = NonNullable<ForgemindStatus['configuration']['mineru']>
type DoclingStatus = NonNullable<ForgemindStatus['configuration']['docling']>
...
const getModelRows = (status: ForgemindStatus): RoleLLMRow[] => {
...
const StatusCard = ({ status }: { status: ForgemindStatus | null }) => {
