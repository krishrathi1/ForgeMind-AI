...
// Types
export type ForgemindNodeType = {
  id: string
  labels: string[]
  properties: Record<string, any>
}

export type ForgemindEdgeType = {
  id: string
  source: string
  target: string
  type: string
...
export type ForgemindGraphType = {
  nodes: ForgemindNodeType[]
  edges: ForgemindEdgeType[]
}

export type ForgemindQueueStatus = {
  available: boolean
  queue_name?: string
  max_async?: number
...
export type ForgemindRoleLLMConfig = {
  binding?: string | null
  model?: string | null
  host?: string | null
  max_async?: number
...
export type ForgemindStatus = {
  ...
  configuration: {
    ...
    role_llm_config?: Record<string, ForgemindRoleLLMConfig>
  ...
  llm_queue_status?: Record<string, ForgemindQueueStatus>
  embedding_queue_status?: ForgemindQueueStatus
  rerank_queue_status?: ForgemindQueueStatus
  keyed_locks?: {
    process_id: number
    cleanup_performed: {
      mp_cleaned: number
...
export type ForgemindDocumentsScanProgress = {
  is_scanning: boolean
  current_file: string
  ...
  refreshTokenPromise = (async () => {
    try {
      ...
      if (response.data.access_token && !response.data.auth_configured) {
        ...
        // Update localStorage
        localStorage.setItem('FORGEMIND-API-TOKEN', newToken);
        // Update auth state
  ...
  async (error: AxiosError) => {
    if (error.response) {
      if (error.response?.status === 401) {
        ...
        const authStore = useAuthStore.getState();
        const currentToken = localStorage.getItem('FORGEMIND-API-TOKEN');
...
export const checkHealth = async (): Promise<
  ForgemindStatus | { status: 'error'; message: string }
> => {
  try {
...
export const getDocumentsScanProgress = async (): Promise<ForgemindDocumentsScanProgress> => {
