...
let apiModule: typeof import('./forgemind')
...
beforeAll(async () => {
  apiModule = await import('./forgemind')
...
describe('queryTextStream — auth headers', () => {
  test('includes Bearer token when stored', async () => {
    storageData.set('FORGEMIND-API-TOKEN', 'test-jwt-token')
...
describe('queryTextStream — guest-token 401 retry', () => {
  test('retries with refreshed guest token on 401', async () => {
    storageData.set('FORGEMIND-API-TOKEN', 'expired-guest-token')
