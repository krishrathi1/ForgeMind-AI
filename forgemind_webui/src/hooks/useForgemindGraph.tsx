...
import { queryGraphs } from '@/api/forgemind'
...
const createSigmaGraph = async (rawGraph: RawGraph | null): Promise<UndirectedGraph | null> => {
  ...
  if (skippedEdges > 0) {
    console.warn(`[useForgemindGraph] ${skippedEdges} edges could not be added to the graph`)
  }
...
const useLightrangeGraph = () => {
  ...
  useEffect(() => {
    ...
    if (!isFetching && !useGraphStore.getState().graphDataFetchAttempted) {
      ...
      if (lastFetchSignatureRef.current === fetchSignature) {
        console.warn(
          '[useForgemindGraph] Suppressed duplicate graph fetch:',
          fetchSignature,
      ...
      dataPromise
        .then(async (result) => {
          ...
          if (!data || !data.nodes || data.nodes.length === 0) {
            ...
            try {
              ...
              console.error('[useForgemindGraph] createSigmaGraph failed (graph build):', buildError)
            ...
            try {
              ...
              console.error(
                '[useForgemindGraph] a store/sigma subscriber threw while applying the new graph:',
                subscriberError
              )
            ...
            console.log(
              `[useForgemindGraph] sigma graph ready: ${newSigmaGraph?.order} nodes, ${newSigmaGraph?.size} edges`
            )
        ...
        .catch((error) => {
          console.error(
            '[useForgemindGraph] graph load failed (see preceding log for stage):',
            error
          )
          ...
          if (error instanceof GraphBuildError) {
            console.error(
              '[useForgemindGraph] graph build failed (not retrying — deterministic):',
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            retryStateRef.current = { signature: '', attempts: 0 }
          ...
          if (retry.attempts < MAX_FETCH_RETRIES) {
            ...
            console.warn(
              `[useForgemindGraph] graph fetch failed, retry ${retry.attempts}/${MAX_FETCH_RETRIES} in ${delay}ms:`,
              fetchSignature
            )
            if (retryTimerRef.current !== null) {
              clearTimeout(retryTimerRef.current)
            ...
            console.error(
              `[useForgemindGraph] graph fetch failed after ${MAX_FETCH_RETRIES} retries, giving up:`,
              fetchSignature
            )
            lastFetchSignatureRef.current = fetchSignature
            toast.error(
    ...
    const handleNodeExpand = async () => {
      ...
      try {
        ...
        for (const edgeId of edgesToAdd) {
          ...
          if (dynamicId === null) {
            console.warn('[useForgemindGraph] could not add expansion edge:', newEdge.id)
            continue
          }
  ...
  const forgemindeGraph = useCallback(() => {
  ...
  return { forgemindeGraph, getNode, getEdge }
}
