...
ForgeMind WebUI is a React-based web interface for interacting with the ForgeMind system. It provides a user-friendly interface for querying, managing, and exploring ForgeMind's functionalities.
...
2. **Install Dependencies:**
    ...
    In the `forgemind_webui` directory, run the following command to install project dependencies:
...
3. **Build the Project:**
    ...
    This command will bundle the project and output the built files to the `forgemind/api/webui` directory.
...
1. **Move the project into the Linux filesystem.** Clone/copy ForgeMind somewhere under your WSL home (e.g. `~/ForgeMind`) instead of `/mnt/c/...`, then reinstall:
